    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (tl.getBoolInput('buildNumber', false)) {
             ...
             if (tl.getBoolInput('buildSourceVersionMsg', false)) {
                 var buildSourceVersionMsg = tl.getVariable("Build.SourceVersionMessage");
                 body += "\n<b>Commit Message:</b> " + buildSourceVersionMsg;
             }
             if (tl.getBoolInput('buildReason', false)) {
                 var buildReason = tl.getVariable("Build.Reason");
                 body += "\n<b>Build Reason:</b> " + buildReason;
             }
             if (tl.getBoolInput('releaseReason', false)) {
                 var releaseReason = tl.getVariable("Release.Reason");
                 body += "\n<b>Release Reason:</b> " + releaseReason;
             }    
             if (tl.getBoolInput('releaseEnv', false)) {
                 var releaseEnv = tl.getVariable("Release.EnvironmentName");
                 body += "\n<b>Release Environment:</b> " + releaseEnv;
             }                       
